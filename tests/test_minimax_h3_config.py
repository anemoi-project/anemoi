...
class MiniMaxH3MPAConfigTests(unittest.TestCase):
    def test_released_json_resolves_to_the_released_policy(self) -> None:
        ...
        self.assertEqual(policy["video_sparsity_ratio"], 0.9)
        self.assertEqual(
            policy["layer_sparsity_bands"],
            ((18, 34, 0.85), (34, 50, 0.65)),
        )
        self.assertAlmostEqual(policy["average_sparse_layer_sparsity_ratio"], 0.8)
        self.assertEqual(policy["dense_first_steps"], 10)
        self.assertEqual(policy["dense_first_layers"], 2)

    def test_json_changes_policy_and_cli_precision_has_priority(self) -> None:
        config = {
            "sparsity_ratio": 0.75,
        ...
        self.assertFalse(policy["adaptive_tile"])

    def test_unknown_json_field_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            path = Path(temporary) / "mpa.json"
            path.write_text(json.dumps({"sparsity": 0.9}))
