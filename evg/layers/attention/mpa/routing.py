...
class RaggedRoutePlan:
    """Compact FP8-then-FP16 lists consumed by the native K64 executor."""
...
def _route_counts(
    ...
    *,
    ...
    """Hamilton-allocate the retained budget as ``(FP16, FP8)``."""

    if type(items) is not int or items <= 0:
        raise ValueError("items must be a positive integer")
    values = (sparsity_ratio, fp8_ratio, fp16_ratio)
    if any(
    ...
    sparsity, fp8, fp16 = map(float, values)
    if not 0.0 <= sparsity < 1.0:
        raise ValueError("sparsity_ratio must be in [0, 1)")
    if fp8 <= 0.0 or fp16 <= 0.0:
        raise ValueError("FP8/FP16 ratios must be positive")
    if not math.isclose(fp8 + fp16, 1.0, rel_tol=0.0, abs_tol=1.0e-6):
        raise ValueError("FP8/FP16 ratios must sum to one")

    retained = min(items, max(1, math.floor((1.0 - sparsity) * items + 0.5)))
    quotas = (retained * fp16, retained * fp8)
    counts = [math.floor(value) for value in quotas]
    order = sorted(range(2), key=lambda index: (-(quotas[index] - counts[index]), index))
...
def route_probability(
    ...
    *,
    ...
    """Global top-k per head with all ragged adjacency anchors in FP16."""
    ...
    nominal_fp16, nominal_fp8 = _route_counts(
        rows * columns,
        sparsity_ratio=sparsity_ratio,
        fp8_ratio=fp8_ratio,
        fp16_ratio=fp16_ratio,
    )
    nominal_retained = nominal_fp16 + nominal_fp8
    # On very small grids, the requested sparse budget can be smaller than the
    # self/shared-boundary set.  Preserve every mandatory anchor and use the
    # minimum feasible budget instead of rejecting an otherwise valid shape.
    retained = max(nominal_retained, anchor_count)
    fp16_per_head = max(nominal_fp16, anchor_count)
    fp8_per_head = retained - fp16_per_head

    priority = probability.masked_fill(
        anchors.view(1, 1, rows, columns),
        torch.finfo(probability.dtype).max,
    )
    selected = torch.topk(
        priority.reshape(batch, heads, -1),
        retained,
        dim=-1,
        largest=True,
        sorted=True,
    ...
    precision.scatter_(
        ...
        torch.full_like(selected[..., :fp16_per_head], 2, dtype=torch.uint8),
    )
    precision.scatter_(
        -1,
        selected[..., fp16_per_head:],
        torch.ones_like(selected[..., fp16_per_head:], dtype=torch.uint8),
    )
    precision = precision.view(batch, heads, rows, columns)
    fp8_counts = (precision == 1).sum(dim=-1, dtype=torch.int32)
    fp16_counts = (precision == 2).sum(dim=-1, dtype=torch.int32)

    column_ids = torch.arange(columns, device=probability.device, dtype=torch.int32).view(
        1, 1, 1, columns
    )
    # Per-row argsort compacts FP8, FP16, then unused ids.
