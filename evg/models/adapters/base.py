...
from evg.models.specs import ModelSpec, ModelVariant
...
class VideoModelAdapter:
    ...
    def generate(self, request: GenerationRequest) -> GeneratedArtifact:
        ...
        raise AdapterError(
            f"{self.spec.name} has an EVG adapter contract, but the executable runner "
            "has not landed yet. Use 'evg inspect' to see the upstream model id."
        )
