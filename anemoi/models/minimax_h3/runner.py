#!/usr/bin/env python3
"""Run one MiniMax-H3 compressed-weight quality candidate on supported GPUs.
...
DEFAULT_RESOURCE_ROOT = Path(
    os.environ.get("ANEMOI_MINIMAX_H3_ROOT", REPO_ROOT / "models/minimax-h3")
)
...
def load_prompt_registry(path: Path) -> dict[str, tuple[Path, str]]:
    payload = json.loads(path.read_text())
    if not isinstance(payload, dict):
        raise TypeError("prompt registry must be a JSON object")
    prompts: dict[str, tuple[Path, str]] = {}
    for prompt_id, spec in payload.items():
        ...
        conditioning = conditioning.resolve()
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--prompt-id", default="official-example-1")
...
def _finite_ratio(value: Any, name: str, *, allow_one: bool = False) -> float:
    ...
    result = float(value)
    in_range = 0.0 <= result <= 1.0 if allow_one else 0.0 <= result < 1.0
    if not math.isfinite(result) or not in_range:
...
def _apply_mpa_config(profile: dict[str, Any], config: dict[str, Any]) -> None:
    ...
    if sm120_requested:
        ...
        if not math.isclose(nvfp4 + int8 + mxfp8 + fp16, 1.0, abs_tol=1e-6):
            raise ValueError("NVFP4/INT8/MXFP8/FP16 ratios must sum to one")
        profile["precision"] = {
            "nvfp4": nvfp4,
            "int8": int8,
            "mxfp8": mxfp8,
    ...
    elif ("fp8_ratio" in config) != ("fp16_ratio" in config):
    ...
    if "layer_precision_bands" in config:
        ...
        for band in bands:
            ...
            if not math.isclose(sum(values), 1.0, abs_tol=1e-6):
                raise ValueError("layer precision ratios must sum to one")
            if values[1] > 0.0 and values[2] > 0.0:
                raise ValueError("INT8 and MXFP8 are alternative middle phases")
            resolved.append((first, last, *values))
            previous_end = last
...
def _resolved_candidate(
    ...
    if fp8_ratio is not None:
        assert fp16_ratio is not None
        if int(profile.get("query_block_size", 64)) != 64:
            raise ValueError("FP8 CLI overrides apply only to Q64")
        profile["precision"] = {
            "fp8": float(fp8_ratio),
...
def _enable_long_sequence_group_offload(
    transformer: Any, sequence_tokens: int
) -> dict[str, Any] | None:
    if sequence_tokens <= LONG_SEQUENCE_OFFLOAD_TOKENS:
        return None
    config = {
        "onload_device": "cuda",
        "offload_device": "cpu",
    ...
    transformer.enable_group_offload(**config)
    return config
...
def _import_benchmark_support():
    return importlib.import_module("anemoi.models.minimax_h3.runtime")
...
def _warm_sparse_provider(
    ...
    *,
    ...
    if candidate["kind"] == "official-sol":
        ...
        os.environ["ANEMOI_MPA_WARMUP_SYNC"] = "1"
        try:
            output = attention.mpa(q, k, v, layer=2)
        finally:
            os.environ.pop("ANEMOI_MPA_WARMUP_SYNC", None)
    ...
    *,
    ...
    return H3MPAAttention(
        H3MPAConfig(
            ...
            mxfp8_ratio=float(precision.get("mxfp8", 0.0)),
...
def _install_post_ulysses(
    ...
    *,
    ...
    if candidate["kind"] == "official-sol":
        ...
        from anemoi.models.minimax_h3 import install_layout_observer
...
def _main_distributed(args: argparse.Namespace, candidate: dict[str, Any]) -> int:
    ...
    output_dir = args.output_dir.resolve()
    delivered_frames, duration_s = delivery_contract(args.num_frames)
    if rank == 0:
    ...
    local = {
        ...
        "transformer_resident_gpu_s": round(sum(resident_samples) / 1000.0, 3),
        "offload_transfer_gpu_s_excluded": round(sum(excluded_samples) / 1000.0, 3),
        "timing_scope": (
            "transformer CUDA span minus synchronous group-offload transfer spans"
            if transfer_timer is not None
            else "transformer CUDA span; no offload"
        ),
        "per_eval_gpu_ms": [round(value, 3) for value in samples],
        "per_eval_resident_gpu_ms": [round(value, 3) for value in resident_samples],
        "per_eval_offload_transfer_gpu_ms_excluded": [
            round(value, 3) for value in excluded_samples
        ],
    ...
    if rank == 0:
        ...
        results = {
            ...
            "fairness": {
                ...
                "attention_precision_note": _attention_precision_note(candidate),
            },
            "provenance": {
...
def main() -> int:
    ...
    if args.prompt_registry is not None:
    ...
    try:
        ...
        raise ValueError(f"unknown --prompt-id {args.prompt_id!r}") from None
    ...
    if args.height <= 0 or args.width <= 0 or args.height % 32 or args.width % 32:
        raise ValueError("height/width must be positive multiples of 32")
    delivery_contract(args.num_frames)
