                                                    ...
                                                    const smem_t<swizzle_mode, stride> &smem)
{
  ...
  static_assert(std::is_same<T, half>::value || std::is_same<T, nv_bfloat16>::value || std::is_same<T, int8_t>::value);
...
__device__ __forceinline__ void compute_half_qk(const smem_t<swizzle_mode, stride> &smem_Q, const smem_t<swizzle_mode, stride> &smem_K, float RS[][num_tiles_k][8], uint32_t &offset_Q, uint32_t &offset_K)
{
...
#pragma unroll
      for (uint32_t fq = 0; fq < num_tiles_q; fq++)
      {
        mma::mma_sync_m16n16k16_row_col_f16f16f32<mma::MMAMode::kInit>(RS[fq][fk], RQ[fq], RK);
      }
    }
...
#pragma unroll
      for (uint32_t fq = 0; fq < num_tiles_q; fq++)
      {
        mma::mma_sync_m16n16k16_row_col_f16f16f32<mma::MMAMode::kInplaceUpdate>(RS[fq][fk], RQ[fq], RK);
      }
    }
    offset_K = smem_K.advance_offset_by_column<2>(offset_K - (num_tiles_k * 16 * stride), iter);
  }
...
__device__ __forceinline__ float update_mo(float RS[][num_tiles_k][8], DTypeSVAccum RO[][num_tiles_v][8], float m[][2], float d[][2], const float &sm_scale)
{
  // first calculate the max and max diff
  float local_max_diff = -5000000.0f;
#pragma unroll
  for (uint32_t fq = 0; fq < num_tiles_q; fq++)
  {
#pragma unroll
